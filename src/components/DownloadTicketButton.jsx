...
export default function DownloadTicketButton({ fileName = "ticket.pdf", ticket, qrSource }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const paidAt = ticket.paymentTime ? formatDateTimeShort(ticket.paymentTime) : "Unavailable";
      const visitDate = ticket.visitDate ? formatShortDate(ticket.visitDate) : "Unavailable";
      ...
      drawInfoCard(pdf, cardX + 24 + leftWidth + gap, contentTop, middleWidth, 182, [
        { label: "VISIT DATE", value: visitDate, valueSize: 11 },
  ...
  return (
    <button type="button" onClick={handleDownload} disabled={downloading} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#5123B6] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#461AA2] disabled:cursor-not-allowed disabled:opacity-60">
