...
export default async function PaymentSuccess({ searchParams }) {
  ...
  const verificationUrl = shouldShowTicket ? getTicketVerificationUrl(ticket.orderId) : "";
      ...
      <div className="mx-auto max-w-5xl">
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <section className="rounded-[32px] bg-white p-6 shadow-[0_20px_70px_rgba(16,24,40,0.08)] md:p-8">
            ...
            {shouldShowTicket ? (
              <div id="ticket-download-card" className="mt-8 overflow-hidden rounded-[28px] border border-[#D4F0DB] bg-[linear-gradient(135deg,#123B2A_0%,#0F766E_55%,#D7F5DD_100%)] p-[1px]">
                <div className="rounded-[27px] bg-[linear-gradient(135deg,#0F172A_0%,#1F2937_55%,#1B4332_100%)] p-6 text-white">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#86EFAC]">Entry Ticket</p>
                      <h2 className="mt-3 text-2xl font-extrabold md:text-3xl">{ticket.note}</h2>
                      <p className="mt-2 text-sm text-white/75">Show this ticket at the gate for verification.</p>
                    </div>
                    ...
                    <div className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-right">
                      ...
                      <p className="text-xl font-bold">{ticket.currency} {ticket.amount}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-8 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="rounded-3xl bg-white/8 p-5 backdrop-blur">
                        <p className="text-xs font-bold uppercase tracking-[0.14em] text-white/60">Guest Name</p>
                        <p className="mt-2 text-xl font-bold">{ticket.customerName}</p>
                        <p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-white/60">Mobile</p>
                        <p className="mt-2 text-base font-semibold">{ticket.customerPhone}</p>
                        <p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-white/60">Email</p>
                        <p className="mt-2 break-all text-sm font-semibold">{ticket.customerEmail}</p>
                      </div>

                      <div className="rounded-3xl bg-white/8 p-5 backdrop-blur">
                        <p className="text-xs font-bold uppercase tracking-[0.14em] text-white/60">Visit Date</p>
                        <p className="mt-2 text-lg font-bold">{ticket.visitDate ? formatShortDate(ticket.visitDate) : "Unavailable"}</p>
                        <p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-white/60">Payment Time</p>
                        <p className="mt-2 text-lg font-bold">{ticket.paymentTime ? formatDateTimeShort(ticket.paymentTime) : "Unavailable"}</p>
                        <p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-white/60">Order Reference</p>
                        <p className="mt-2 break-all font-mono text-sm">{ticket.orderId}</p>
                        <p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-white/60">Ticket ID</p>
                        <p className="mt-2 break-all font-mono text-sm">{ticket.ticketId}</p>
                        <p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-white/60">Ticket Status</p>
                        <p className="mt-2 text-sm font-semibold uppercase tracking-[0.14em] text-[#86EFAC]">{ticket.status === "redeemed" ? "Redeemed" : "Valid for Entry"}</p>
                      </div>
                    </div>

                    <div className="rounded-3xl bg-white/8 p-5 text-center backdrop-blur">
                      <img src={qrSource} alt={`QR ticket for ${ticket.customerName}`} crossOrigin="anonymous" className="mx-auto h-44 w-44 rounded-2xl bg-white p-3 shadow-sm" />
                      <p className="mt-4 text-sm font-semibold text-[#CFFCC5]">Scan to verify this ticket</p>
                      <p className="mt-1 text-xs text-white/65">Opens the live ticket verification page.</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <aside className="space-y-6">
            {shouldShowTicket ? <TicketQrCard verificationUrl={verificationUrl} customerName={ticket.customerName} /> : null}
            ...
            <div className="rounded-[28px] bg-white p-6 shadow-[0_18px_45px_rgba(24,34,69,0.08)]">
              ...
              <p className="mt-3 text-sm text-gray-600">Scan the QR at entry. The verification page will show the customer details and a green verified tick when the payment is successful.</p>
            </div>
          </aside>
        </div>
      </div>
